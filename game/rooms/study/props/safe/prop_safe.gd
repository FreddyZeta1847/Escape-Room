...
func _on_room_set() -> void:
	if not CombinationLock.lock_solved.is_connected(_on_safe_opened):
...
func _on_right_click() -> void:
	await C.player.say("A heavy safe with a 4-digit combination lock.")
...
func _on_safe_opened() -> void:
	await C.player.say("The safe opened! There's a key inside!")
