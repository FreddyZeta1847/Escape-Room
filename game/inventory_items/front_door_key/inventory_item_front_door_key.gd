...
const Data := preload('inventory_item_front_door_key_state.gd')

var state: Data = load("res://game/inventory_items/front_door_key/inventory_item_front_door_key.tres")

## Attributes used by the InteractionSystem for attribute-based locks.
var attributes := ["front_door_key"]
